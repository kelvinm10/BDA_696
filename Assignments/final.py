...
def extract_significant_features(pvalues, features, threshold, filter=False):
    ...
    feature_index = []
    if filter:
        temp = ["temp_diff_home", "temp_diff_away", "wind_diff_home", "wind_diff_away"]
    else:
        temp = []
    for i in pvalues:
        if idx == 0:
...
def main():
    ...
    query = """
              SELECT *
              FROM final_individual_features;
        """
    ...
    significant_feat = extract_significant_features(
        pvalues, features, 0.37, filter=False
    )
    ...
    lr_search = logit(
        ...
        y_train,
        retrain=False,
        model_path="models/tuned_logit_individual.sav",
    )
    ...
    rf_search = random_forest(
        ...
        y_train,
        retrain=False,
        model_path="models/tuned_rf_individual.sav",
    )
    ...
    svc_model = svc(
        ...
        y_train,
        retrain=False,
        model_path="models/svc_individual.sav",
    )
